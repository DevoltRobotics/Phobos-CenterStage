...
class DeltaScheduler internal constructor() {
    ...
    private val subsystemsDefMap = mutableMapOf<DeltaSubsystem, DeltaCommand?>()

    val subsystems get() = subsystemsDefMap.keys.toTypedArray()
    ...
    //user events
    private val initEvents = mutableListOf<DeltaSchedulerEvent>()
    private val runEvents = mutableListOf<DeltaSchedulerEvent>()
    private val interruptEvents = mutableListOf<DeltaSchedulerEvent>()
    private val endEvents = mutableListOf<DeltaSchedulerEvent>()
    private val runSchedulerEvents = mutableListOf<Runnable>()
    ...
    fun addSubsystem(vararg subsystems: DeltaSubsystem) {
        for(subsystem in subsystems) {
            this.subsystemsDefMap[subsystem] = null
        }
    }
    ...
    fun removeSubsystem(vararg subsystems: DeltaSubsystem) {
        for(subsystem in subsystems) {
            this.subsystemsDefMap.remove(subsystem)
        }
    }
    ...
    fun update() {
        ...
        for(subsystem in subsystemsDefMap.keys) { subsystem.internalUpdate() } //run the loop method of all the subsystems
        ...
        for((subsystem, defCmd) in subsystemsDefMap) {
    ...
    fun setDefaultCommand(subsystem: DeltaSubsystem, command: DeltaCommand) {
        ...
        subsystemsDefMap[subsystem] = command
    }
     ...
     */
    fun getDefaultCommand(subsystem: DeltaSubsystem) = subsystemsDefMap[subsystem]
